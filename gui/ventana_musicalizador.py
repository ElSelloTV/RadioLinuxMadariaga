...
from PySide6.QtWidgets import (
    ...
    QPushButton, QLabel, QLineEdit, QMessageBox, QInputDialog, QAbstractItemView
)
...
def texto_resumen_item(item_config: dict) -> str:
    tipo = item_config.get("tipo")
    if tipo == TIPO_ESPECIFICO:
        base = f"🎯 Específico: {item_config.get('ruta', '(sin archivo)')}"
    elif tipo == TIPO_ALEATORIO:
        ruta = " > ".join(item_config.get("categoria") or []) or "(sin categoría)"
        sufijo = "" if item_config.get("recursivo", True) else " (sin subcategorías)"
        base = f"🎲 Aleatorio: {ruta}{sufijo}"
    elif tipo == TIPO_SUBFORMATO:
        minutos = int((item_config.get("duracion_segundos") or 0) / 60)
        base = f"🔁 Subformato: {item_config.get('nombre', '(sin formato)')} ({minutos} min)"
    else:
        base = "(ítem desconocido)"

    if item_config.get("pisador_categoria"):
        posicion = "Outro" if item_config.get("pisador_posicion") == "final" else "Intro"
        base += f"  · Pisador ({posicion}): {' > '.join(item_config['pisador_categoria'])}"
    return base
...
class VentanaMusicalizador(QDialog):
    ...
    def _construir_ui(self):
        ...
        self.lista_items.itemDoubleClicked.connect(lambda _item: self._editar_item())
    ...
    def _refrescar_lista_items(self):
        ...
        for item_config in self._items_en_edicion:
            item_ui = QListWidgetItem(texto_resumen_item(item_config))
            item_ui.setData(ROL_ITEM_CONFIG, item_config)
            self.lista_items.addItem(item_ui)
    ...
    def _editar_item(self):
        ...
        if item:
            ...
            self._refrescar_lista_items()
            self.lista_items.setCurrentRow(fila)
    ...
    def _mover_item(self, delta: int):
        ...
        self._refrescar_lista_items()
        self.lista_items.setCurrentRow(nueva_fila)
